"""Run: python resnet_scripts/05_train_resnet50.py.
...
BEST_LOSS_PATH = MODELS_DIR / "resnet50_best_loss.pth"
BEST_ACCURACY_PATH = MODELS_DIR / "resnet50_best_accuracy.pth"
HISTORY_PATH = PROJECT_ROOT / "results" / "metrics" / "resnet50_history.csv"
PLOTS_DIR = PROJECT_ROOT / "results" / "plots"

MODEL_NAME = "torchvision.models.resnet50"
...
def build_model():
    ...
    model.fc = nn.Sequential(nn.Dropout(CLASSIFIER_DROPOUT), nn.Linear(in_features, 2))
    print(f"Pretrained weights loaded: ResNet50_Weights.{WEIGHTS.name}")
    return model
...
def configure_stage2(model):
    ...
    # Early ImageNet layers encode edges, gradients, textures and simple shapes.
    # Adapt only the high-level layer4 features to fundus morphology to limit
    # overfitting and catastrophic forgetting. In particular, layer3 stays frozen.
    model.layer4.requires_grad_(True)


def verify_trainable_parameters(model, stage):
    """Require exactly fc in Stage 1 and exactly layer4 + fc in Stage 2."""
    expected = {id(parameter) for parameter in model.fc.parameters()}
    if stage == 2:
        expected.update(id(parameter) for parameter in model.layer4.parameters())
    elif stage != 1:
    ...
    print(
        ...
        + ("fc only." if stage == 1 else "layer4 + fc only.")
    )


def set_frozen_batchnorm_eval(model):
    """Freeze running statistics wherever the BatchNorm parameters are frozen."""
...
def set_training_mode(model):
    model.train()
    for name in BACKBONE_BLOCKS:
        block = getattr(model, name)
        if not any(parameter.requires_grad for parameter in block.parameters()):
            block.eval()
    # Reapply this after every model.train(). During Stage 2, layer4 BatchNorm
    # learns normally; stem/layer1/layer2/layer3 running statistics stay unchanged.
...
def experiment_metadata(train_loader, val_loader):
    ...
    return {
        ...
        "pretrained_weights": f"ResNet50_Weights.{WEIGHTS.name}",
        ...
        "training_config": {
            ...
            "fine_tuned_blocks": ["layer4", "fc"],
            "batchnorm_running_statistics": "frozen except layer4 during Stage 2",
            "lr_factor": 0.5,
...
def save_checkpoint(model, path, metrics, metadata, selection_metric):
    ...
    print(
        ...
        f"Layer4 LR: {metrics['learning_rate_backbone']:.2e} | "
...
def plot_history(history):
    ...
    for metric, ylabel in [
        ...
        if metric == "learning_rate":
            ...
            if backbone_epochs:
                ax.step(backbone_epochs, backbone_rates, where="post", label="Layer4")
            ax.set_yscale("log")
        else:
        ...
        ax.set(xlabel="Global epoch", ylabel=ylabel, title=f"ResNet50 - {ylabel}")
        ax.grid(True, alpha=0.3)
        ax.legend()
        fig.tight_layout()
        fig.savefig(PLOTS_DIR / f"resnet50_{metric}_curve.png", dpi=300)
        plt.close(fig)
...
def main():
    ...
    print(f"Model: ResNet50 ({MODEL_NAME}) | Weights: {WEIGHTS.name}")
    ...
    with HISTORY_PATH.open("w", newline="", encoding="utf-8") as history_file:
        ...
        for stage, max_epochs in [(1, STAGE1_EPOCHS), (2, STAGE2_EPOCHS)]:
            if stage == 1:
                ...
                configure_stage2(model)
                print(
                    "Limited fine tuning: layer4 + fc; conv1, bn1, layer1, layer2, layer3 remain frozen."
                )
            ...
            trainable_blocks = [
                name
                for name in (*BACKBONE_BLOCKS, "fc")
                if any(p.requires_grad for p in getattr(model, name).parameters())
            ]
            frozen_blocks = [
                name for name in (*BACKBONE_BLOCKS, "fc") if name not in trainable_blocks
            ]
            print(f"  Frozen blocks: {', '.join(frozen_blocks)}")
            print(f"  Trainable blocks: {', '.join(trainable_blocks)}")
            ...
            for epoch in range(1, max_epochs + 1):
                ...
                print(
                    ...
                    f"Layer4 LR: {row['learning_rate_backbone']:.2e} | "
